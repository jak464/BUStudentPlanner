...
studentDashboard.render = function(req, res) {
    db.query('select' +
        ...
        ' left join assignment_student ast on ast.assignment_id = asg.id and ast.student_id = s.id and ast.can_view = true ' +
        ' where cs.is_subscribed = true ' +
        ...
        function(err, result) {
            ...
            for(var i = 0; i < result.rows.length; i++) {
                if(result.rows[i].assignment_id && result.rows[i].can_view) {
